...
if command -v pacman &> /dev/null; then
    ...
    sudo dnf install -y \
        ...
        arm-none-eabi-gcc-cs arm-none-eabi-newlib arm-none-eabi-gdb openocd avr-gcc avr-libc avrdude picocom minicom \
        git curl wget neovim alacritty \
...
echo "==> 2. Setting up Hardware / Udev permissions (for STM32, Arduino, ST-Link, USB-UART)..."
# Add current user to dialout/uucp group for serial port access without sudo
...
echo "==> 4. Setting up LazyVim (C/C++ & Embedded LSP)..."
mkdir -p "$HOME/.config"
...
echo "==> 5. Setting up Starship & Omarchy-style Shell Aliases..."
...
echo "==> 6. Setting up Alacritty (Omarchy Tokyo Night Theme)..."
...
echo "==> 7. Configuring GNOME Theme & Styling..."
if command -v gsettings &> /dev/null; then
    ...
    curl -sSLo "$WALLPAPER_PATH" "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/wallpapers/wallpaper.jpg" || true
...
echo "==> 8. Installing Desktop GUI Applications via Flatpak..."
...
echo "=========================================================="
echo " Setup complete! GNOME Dark + Embedded C/C++ + LazyVim    "
echo " (Note: Log out & log back in to apply serial/dialout USB "
